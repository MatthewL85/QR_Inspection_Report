...
| Document template ownership | P0 | Foundation | Core Platform plus document-owning module | Shared renderer can be reused, but document ownership stays with the module that creates the document. Work orders are Works/LogixPM documents; job dockets, quotations and payment requests are Contractor Logix documents. |
...
| Finance Logix | Foundation | Budgets, service charges, debtors, arrears, aged debtor snapshots, ledgers, invoices, payments, balances and finance documents | Yes. It may connect to Works/Contractor payment requests through `work_order_id`, but owns finance records and must remain usable as a standalone module through `company_id`, `client_id`, `unit_id` and future `member_id` visibility. | Governed invoice approval, ledger posting, payment runs, bank reconciliation, chart of accounts depth, invoice templates, accounting integrations and member-facing owner balance rules. |
