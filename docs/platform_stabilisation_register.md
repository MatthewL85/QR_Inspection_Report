...
The close-out sign-off checklist is `docs/stabilisation_security_closeout.md`. Treat it as the short release-readiness control before moving from stabilisation into wider feature expansion. The role-by-module access contract is `docs/module_access_matrix.md`. The document-template ownership contract is `docs/document_template_ownership_matrix.md`.
